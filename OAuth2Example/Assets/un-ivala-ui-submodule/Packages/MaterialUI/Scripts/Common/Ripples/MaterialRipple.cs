...
namespace MaterialUI
{
    ...
    public class MaterialRipple : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler, IPointerExitHandler, IRippleCreator, ISelectHandler, IDeselectHandler, ISubmitHandler
    {
        ...
        /// </summary>
		/// 
		// [SS] BEGIN Added HoveredOrToggled
        ...
        private void Update()
        {
			...
			// [SS] BEGIN
			if ( !m_Hovered
				&& highlightWhen == HighlightActive.HoveredOrToggled
				&& m_Toggle != null 
				&& !m_Toggle.isOn )
			{
				Highlight(2);
...
// [SS] END
        ...
        public void OnPointerDown(PointerEventData eventData)
        {
            ...
            else
            {
                ...
                // [SS] BEGIN
                else if(highlightWhen == HighlightActive.HoveredOrToggled)
                {
                    Highlight(  IsToggled() ? 1 : 2 );
                }
                // [SS] END
            }
        }
...
// [SS] BEGIN
		...
		{
			bool Result = false;
			if ( highlightWhen == HighlightActive.HoveredOrToggled )
			{
				if ( m_Toggle == null )
				{
        ...
        public void HighlightBool( bool bHighlight )
        {
            Highlight( bHighlight ? 1 : 2 );
        }
// [SS] END
...
// [SS] BEGIN
			if (highlightWhen == HighlightActive.Hovered 
				|| (highlightWhen == HighlightActive.HoveredOrToggled && !IsToggled() ) )
            {
                Highlight(2);
            }
// [SS] END
