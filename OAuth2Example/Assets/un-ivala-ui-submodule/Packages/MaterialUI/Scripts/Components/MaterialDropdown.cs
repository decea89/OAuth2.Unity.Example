...
namespace MaterialUI
{
    ...
    public class MaterialDropdown : UIBehaviour, IOptionDataListContainer
    {
		...
		//SS_BEGIN
		[SerializeField]
		private string m_DropdownPanelPrefabName = PrefabManager.ResourcePrefabs.dropdownPanel;
		//SS_END
        ...
        /// </summary>
        // [SS] BEGIN
        public RectTransform dropdownPanel { get { return m_DropdownPanel; } }
        // [SS] END
		...
		private Canvas dropdownCanvas { get {
				if ( m_DropdownCanvas == null ){
					m_DropdownCanvas = m_BaseTransform.root.GetComponent<Canvas>().Copy(dropdownCanvasGameObject);
				}
				return m_DropdownCanvas;
			} }
        /// <summary>
		...
		// [SS] BEGIN
		public bool isExpanded { get { return m_IsExapanded; } }
		// [SS] END
        /// <summary>
		...
		private GameObject dropdownCanvasGameObject { get{
				...
				return m_DropdownCanvasGameObject;
			} }
        ...
        {
			// SS_BEGIN m_DropDownCanvasGameObject is set in dropdownCanvasGameObject getter
            /*Canvas[] canvasses = FindObjectsOfType<Canvas>();
			...
			// SS_BEGIN m_DropdownCanvas is set in the dropdownCanvas getter
            //m_DropdownCanvas = m_BaseTransform.root.GetComponent<Canvas>().Copy(m_DropdownCanvasGameObject);
			// SS_END
        }

		// [SS] BEGIN
		protected void CreateListItemTemplate()
		{
            ...
            m_ListItemTemplate.text = m_ListItemTemplate.rectTransform.GetChildByName<TMPro.TextMeshProUGUI>("Text");
            // [SS] END

			if (m_OptionDataList.imageType == ImageDataType.Sprite)
			{
				// SS_BEGIN
				Graphic icon = m_ListItemTemplate.rectTransform.GetChildByName<Image>("Icon");
				if ( icon != null )
				{
					m_ListItemTemplate.image = icon;
					Destroy(icon);
				}          
				// SS_END
			}
			...
			{
				// SS_BEGIN
				Graphic image = m_ListItemTemplate.rectTransform.GetChildByName<VectorImage>("Icon");
				if ( image != null )
				{					
					m_ListItemTemplate.image = image;
					Destroy(image);
				}
				//SS_END
			}
        ...
        {
			// SS_BEGIN
            // destroying old tweens
            if ( m_AutoTweeners != null )
            {
			...
			m_BaseTransform.root.GetComponent<Canvas>().CopySettingsToOtherCanvas(dropdownCanvas);
			...
			m_DropdownPanel = PrefabManager.InstantiateGameObject(m_DropdownPanelPrefabName, dropdownCanvas.transform).GetComponent<RectTransform>();
            // SS_END

            _scrollBar = m_DropdownPanel.GetComponentInChildren<Scrollbar>();
			...
			// [SS] BEGIN
			// Moved to CreateListItemTemplate function
			...
			m_ListItemTemplate.rectTransform.gameObject.SetActive(false);
		...
		public void Refresh()
		{
			// destroying all previously created items
			for ( int i = 0; i < m_ListItems.Count; i++ )
			{
                if ( m_ListItems[i] != null && m_ListItems[i].rectTransform != null )
                {                    
                    Destroy(m_ListItems[i].rectTransform.gameObject);
                }
			}
			...
			m_ListItemTemplate.rectTransform.gameObject.SetActive(true);
			...
			m_DropdownCanvasGroup.alpha = 1.0f;
			m_ShadowCanvasGroup.alpha = 1.0f;
			m_DropdownPanel.sizeDelta = m_ExpandedSize;
			m_DropdownPanel.position = m_ExpandedPosition;
			//m_AutoTweeners.Add(TweenManager.TweenFloat(f => m_DropdownCanvasGroup.alpha = f, m_DropdownCanvasGroup.alpha, 1f, AnimDuration * 0.66f, 0, null, false, Tween.TweenType.Linear));
			//m_AutoTweeners.Add(TweenManager.TweenFloat(f => m_ShadowCanvasGroup.alpha = f, m_ShadowCanvasGroup.alpha, 1f, AnimDuration * 0.66f, 0, null, false, Tween.TweenType.Linear));

			//m_AutoTweeners.Add(TweenManager.TweenVector2(vector2 => m_DropdownPanel.sizeDelta = vector2, m_DropdownPanel.sizeDelta, m_ExpandedSize, AnimDuration, AnimDuration / 3, null, false, Tween.TweenType.EaseInOutQuint));

			/*m_AutoTweeners.Add(TweenManager.TweenVector3(vector3 => m_DropdownPanel.position = vector3, m_DropdownPanel.position, m_ExpandedPosition, AnimDuration, AnimDuration / 3, () =>
				{
					if (m_BaseSelectable != null && m_IsExapanded)
					{
						m_BaseSelectable.interactable = false;
			...
			for (int i = 0; i < m_ListItems.Count; i++)
			{
				...
				//m_ListItemAutoTweeners.Add(TweenManager.TweenFloat(f => canvasGroup.alpha = f, canvasGroup.alpha, 1f, AnimDuration * 1.66f, (i1 * (AnimDuration / 6) + AnimDuration) - m_ScrollPosOffset / 800, null, false, Tween.TweenType.Linear));
			}

			if (m_FullHeight > m_TempMaxHeight)
			{
				ScrollRect rct = m_DropdownPanel.GetChildByName<ScrollRect>("ScrollRect");
				if ( rct != null )
				{					
					RectMask2D mask = rct.gameObject.GetComponent<RectMask2D>();
					if ( mask == null )
					{
						rct.gameObject.AddComponent<RectMask2D>();	
					}
				}
			}
		}
        ...
        public void Hide()
        {
            ...
            for (int i = 0; i < m_ListItems.Count; i++)
            {
				...
				if ( canvasGroup != null )
				{					
					m_AutoTweeners.Add(TweenManager.TweenFloat(f => canvasGroup.alpha = f, canvasGroup.alpha, 0f, m_AnimationDuration * 0.66f, (m_ListItems.Count - i1) * (m_AnimationDuration / 6), null, false, Tween.TweenType.Linear));
				}
				// [SS] END
            }
	        // [SS] BEGIN
        ...
        private DropdownListItem CreateItem(OptionData data, int index)
        {
            ...
            item.text = item.rectTransform.GetChildByName<TMPro.TextMeshProUGUI>("Text");
            // [SS] END
            ...
            if (m_OptionDataList.imageType == ImageDataType.Sprite)
            {
				...
				// SS_BEGIN
				VectorImage vectorImg = item.rectTransform.GetChildByName<VectorImage>("Icon");
				if ( vectorImg != null )
				{
					Destroy( vectorImg.gameObject );
				}
				// SS_END
            }
            else
            {
				...
				// SS_BEGIN
				Image img = item.rectTransform.GetChildByName<Image>("Icon");
				if ( img != null )
				{
...
// [SS] BEGIN
		...
		{
			if (m_HighlightCurrentlySelected && m_CurrentlySelected >= 0)
			{
				int nActiveIndex = 0;
				for ( int i = 0; i < m_ListItems.Count; i++ )
				{
					DropdownListItem item = m_ListItems[i];
					if (item.rectTransform.gameObject.activeSelf )
					{
					    var isSelected = nActiveIndex == m_CurrentlySelected;

                        if (isSelected)
                        {
                            _scrollBar.value = 1 - nActiveIndex / (float) (m_ListItems.Count - 1); //todo invert the direction of the scrollbar
                        }

                        float fAlpha = isSelected ? m_ItemRippleData.EndAlpha : 0.0f;
					    item.rectTransform.gameObject.GetComponent<Image>().color = m_ItemRippleData.Color.WithAlpha(fAlpha);
        ...
        private float CalculateMaxItemWidth()
        {
            ...
            for (int i = 0; i < m_OptionDataList.options.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(m_OptionDataList.options[i].text))
                {
                    ...
                    currentWidth = m_ListItemTemplate.text.GetPreferredValues(m_OptionDataList.options[i].text).x / scaler.scaler.scaleFactor;
                    // [SS] END
                    currentWidth += 16;
                }
