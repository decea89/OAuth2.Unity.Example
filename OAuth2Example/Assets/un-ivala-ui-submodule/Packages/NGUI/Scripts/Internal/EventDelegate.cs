...
public class EventDelegate
{
	...
	// mergeme begin
	[System.NonSerialized] System.WeakReference[] mCustomArgs;
	public System.WeakReference[] customArgs {
		get {
			return mCustomArgs;
		}
		set{
...
// #endif
			...
			else
			{
				...
				if ( customArgs == null || customArgs.Length == 0 )
				{
					...
					// Allocate the parameter array
					if (mArgs == null || mArgs.Length != mCustomArgs.Length)
						mArgs = new object[mCustomArgs.Length];
					
					// Set all the parameters
					for (int i = 0, imax = mCustomArgs.Length; i < imax; ++i)
					{
						mArgs[i] = mCustomArgs[i].Target as object;
					}
				}
				// mergeme end
