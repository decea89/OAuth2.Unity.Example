...
public class UILabel : UIWidget
{
	...
	// [ss] merge me
	[HideInInspector][SerializeField] int mMaxHeight = 0; // not used if 0
	public int maxHeight { get{ return mMaxHeight; } set { mMaxHeight = value; shouldBeProcessed = true; } }
	// [ss] merge end
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
				...
				else if (mOverflow == Overflow.ResizeHeight)
				{
					...
					// [ss] merge me
					if ( (mMaxHeight > 0) && (mHeight > mMaxHeight) && (--ps > 1)) continue;
					// [ss] merge end
				}
				else
				{
